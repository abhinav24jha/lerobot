...
def main():
    ...
    try:
        ...
        default_arm1_port = "/dev/tty.usbmodem5A7A0589931"
        default_arm2_port = "/dev/tty.usbmodem5A7A0590771"
        arm1_port = input(
            f"Please enter the LEFT arm USB port for arm1 (default: {default_arm1_port}): "
        ).strip()
        arm2_port = input(
            f"Please enter the RIGHT arm USB port for arm2 (default: {default_arm2_port}): "
        ).strip()
        if not arm1_port:
            arm1_port = default_arm1_port
            print(f"Using default left arm port for arm1: {arm1_port}")
        if not arm2_port:
            arm2_port = default_arm2_port
            print(f"Using default right arm port for arm2: {arm2_port}")
        arm1_id = "left_follower_arm"
        arm2_id = "right_follower_arm"

        print(f"Configuring left arm (arm1): {arm1_port}")
        print(f"Configuring right arm (arm2): {arm2_port}")
        
        # Create dual-arm robot instances
        # Use explicit IDs so each arm loads its own existing calibration file.
        arm1_config = SO101FollowerConfig(port=arm1_port, id=arm1_id)
        arm2_config = SO101FollowerConfig(port=arm2_port, id=arm2_id)
