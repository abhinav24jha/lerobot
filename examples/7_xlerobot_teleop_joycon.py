...
def prompt_runtime_config():
    default_robot_id = "my_xlerobot"
    default_port1 = "/dev/ttyACM0"
    default_port2 = "/dev/ttyACM1"

    print("[SETUP] This script expects an XLerobot layout:")
    print("[SETUP] - port1 = left arm + head bus")
    print("[SETUP] - port2 = right arm + base bus")
    print("[SETUP] - one composite calibration file stores left arm, right arm, head, and base")

    robot_id = input(f"[SETUP] Enter XLerobot calibration ID (default: {default_robot_id}): ").strip()
    if not robot_id:
        robot_id = default_robot_id

    port1 = input(f"[SETUP] Enter port1 for LEFT arm + head bus (default: {default_port1}): ").strip()
    if not port1:
        port1 = default_port1

    port2 = input(f"[SETUP] Enter port2 for RIGHT arm + base bus (default: {default_port2}): ").strip()
    if not port2:
        port2 = default_port2

    calibration_dir = HF_LEROBOT_CALIBRATION / ROBOTS / "xlerobot"
    calibration_fpath = calibration_dir / f"{robot_id}.json"

    print(f"[SETUP] Robot ID: {robot_id}")
    print(f"[SETUP] Left arm + head bus (port1): {port1}")
    print(f"[SETUP] Right arm + base bus (port2): {port2}")
    print(
        f"[SETUP] Composite calibration file: {calibration_fpath} "
        f"({'found' if calibration_fpath.is_file() else 'missing'})"
    )
    print("[SETUP] Left Joy-Con controls the left arm and head.")
    print("[SETUP] Right Joy-Con controls the right arm and base.")
    print("[SETUP] Left Joy-Con: left arm on stick/trigger/ZL, head on D-pad.")
    print("[SETUP] Right Joy-Con: right arm on stick/trigger/ZR, base on X/B/Y/A.")

    return robot_id, port1, port2, calibration_fpath


def main():
    FPS = 30
    
    robot_id, port1, port2, calibration_fpath = prompt_runtime_config()
    robot_config = XLerobotConfig(id=robot_id, port1=port1, port2=port2)
    robot = XLerobot(robot_config)
    
    try:
        robot.connect()
        print(f"[MAIN] Successfully connected to robot")
        print(f"[MAIN] Calibration file path: {calibration_fpath}")
