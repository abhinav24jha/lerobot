...
CALIBRATION_FILES = {
    "left": Path(
        "/Users/abhinavjha/.cache/huggingface/lerobot/calibration/robots/so101_follower/left_follower_arm.json"
    ),
    "right": Path(
        "/Users/abhinavjha/.cache/huggingface/lerobot/calibration/robots/so101_follower/right_follower_arm.json"
    ),
}


def parse_args():
    parser = argparse.ArgumentParser(description="SO100/SO101 keyboard end-effector control")
    side_group = parser.add_mutually_exclusive_group()
    side_group.add_argument(
        "--left",
        action="store_true",
        help="Use left arm calibration file (default)",
    )
    side_group.add_argument(
        "--right",
        action="store_true",
        help="Use right arm calibration file",
    )
    return parser.parse_args()


def resolve_calibration_file(args):
    if args.right:
        return CALIBRATION_FILES["right"], "right"
    return CALIBRATION_FILES["left"], "left"
        ...
        try:
                    ...
                    if key in joint_controls:
                        ...
                        if joint_name in target_positions:
                            ...
                            if joint_name == 'gripper':
                                new_target = clamp(current_target + delta * GRIPPER_STEP, GRIPPER_MIN, GRIPPER_MAX)
                            else:
            ...
            for joint_name, target_pos in target_positions.items():
                if joint_name in current_positions:
                    current_pos = current_positions[joint_name]
                    if joint_name == "gripper":
                        target_pos = clamp(target_pos, GRIPPER_MIN, GRIPPER_MAX)
                    error = target_pos - current_pos
                    
                    # P control: output = Kp * error
                    kp_value = GRIPPER_KP if joint_name == "gripper" else kp
                    control_output = kp_value * error
                    
                    # Convert control output to position command
                    new_position = current_pos + control_output
                    if joint_name == "gripper":
                        new_position = clamp(new_position, GRIPPER_MIN, GRIPPER_MAX)
                    robot_action[f"{joint_name}.pos"] = new_position
...
def main():
    """Main function"""
    args = parse_args()
    calibration_file, arm_side = resolve_calibration_file(args)
    ...
    try:
        ...
        # Configure robot and choose calibration file by arm side flag.
        ...
        print(f"Using {arm_side} arm calibration file: {calibration_file}")
